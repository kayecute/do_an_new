...
import Preprocess
import math
# Địa chỉ IP của điện thoại chạy Droidcam
phone_ip = "192.168.1.210"
port = 4747
...
# Đọc video
# Kết nối với camera của điện thoại thông qua Droidcam
cap = cv2.VideoCapture(f"http://{phone_ip}:{port}/video")
#cap = cv2.VideoCapture(f"http://localhost:8080/video")
...
while (cap.isOpened()):
    ...
    # imgcopy = cv2.resize(img, (800, 600))   
    imgcopy = cv2.resize(img, (800, 600), fx=0.5, fy=0.5)
